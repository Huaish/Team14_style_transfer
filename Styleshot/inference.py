...
def main(args):
    ...
    if not os.path.isdir(styleshot_model_path):
        styleshot_model_path = snapshot_download(styleshot_model_path, local_dir=os.path.join("cache", styleshot_model_path))
        print(f"Downloaded model to {styleshot_model_path}")

    # weights for ip-adapter and our content-fusion encoder
    if not os.path.isdir(base_model_path):
        base_model_path = snapshot_download(base_model_path, local_dir=os.path.join("cache", base_model_path))
        print(f"Downloaded model to {base_model_path}")
    if not os.path.isdir(transformer_block_path):
        transformer_block_path = snapshot_download(transformer_block_path, local_dir=os.path.join("cache", transformer_block_path))

...
def calculate_ssim(content_image_path, generated_image_path):
    content_image = cv2.imread(content_image_path)
    generated_image = cv2.imread(generated_image_path)
    scores = []
    for channel in range(3):  # 遍歷 RGB 通道
        score, _ = ssim(content_image[:, :, channel], generated_image[:, :, channel], full=True)
        scores.append(score)
    return np.mean(scores)  # 返回三個通道的平均 SSIM
        ...
        for content_dir in content_dirs:
            ...
            for content_num in content_nums:
                for style_dir in style_dirs:
                    ...
                    data.append(calculate_ssim(content_image_path, generated_image_path))
        data = np.array(data)
        mean = np.mean(data)
        stdev = np.std(data)
        result.append(f'{mean:.3f} ± {stdev:.3f}')
    df = pd.DataFrame({'SSIM': result}, index=[preprocess_types])
    df.to_excel("results_SSIM.xlsx", index=True)
